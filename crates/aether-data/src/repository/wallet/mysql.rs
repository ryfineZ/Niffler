...
"#,
    ...
    async fn load_payment_orders(&self) -> Result<Vec<StoredAdminPaymentOrder>, DataLayerError> {
        let rows = sqlx::query(
            r#"
SELECT
  id, order_no, wallet_id, user_id, amount_usd, debt_repayment_usd, pay_amount, pay_currency,
...
VALUES (?, ?, ?, ?, ?, 0, ?, ?, ?, 0, 0, ?, ?, ?, 'wallet_recharge', 'pending', ?, ?, 'pending', ?, ?)
"#,
        )
        .bind(&order_id)
        .bind(&input.order_no)
...
"#,
        ...
        if has_other_overlapping_plan_mysql(
            &mut tx,
            &input.user_id,
            &input.product_id,
            requested_starts_at,
        ...
        if has_pending_other_plan_purchase_mysql(&mut tx, &input.user_id, &input.product_id, now)
            .await?
        {
            tx.commit().await.map_sql_err()?;
            return Ok(CreatePlanPurchaseOrderOutcome::OverlappingPlanExists);
...
"#,
        ...
        let order_row = if let Some(order_no) = lookup_order_no.as_deref() {
            ...
            mysql_payment_order_by_gateway_order_id_for_update(&mut tx, gateway_order_id).await?
        } else {
            None
        };
...
WHERE user_id = ?
  ...
  AND status = 'active'
                ...
                apply_plan_debt_repayment_mysql(
                    &mut tx,
                    &order_wallet_id,
                    &order_id,
...
VALUES (?, ?, ?, ?, 'active', ?, ?, ?, ?, ?)
                ...
                .await
                .map_sql_err()?;
                save_entitlement_providers_mysql(
                    &mut tx,
                    &entitlement_id,
...
"#,
    ...
    async fn cancel_payment_order(
        ...
        .await
        .map_sql_err()?;
        let updated = map_payment_order_row(&mysql_payment_order_by_id(&mut tx, &order.id).await?)?;
        tx.commit().await.map_sql_err()?;
        Ok(WalletMutationOutcome::Applied((updated, true)))
    }
...
VALUES (?, ?, ?, ?, ?, 0, NULL, NULL, NULL, 0, ?, ?, ?, ?, 'credited', ?, ?, ?)
"#,
        )
        .bind(&order_id)
        .bind(&input.order_no)
...
async fn has_other_overlapping_plan_mysql(
    tx: &mut sqlx::Transaction<'_, sqlx::MySql>,
    user_id: &str,
    plan_id: &str,
    starts_at: i64,
...
async fn has_pending_other_plan_purchase_mysql(
    tx: &mut sqlx::Transaction<'_, sqlx::MySql>,
    user_id: &str,
    plan_id: &str,
    now: i64,
...
async fn save_entitlement_providers_mysql(
    tx: &mut sqlx::Transaction<'_, sqlx::MySql>,
    entitlement_id: &str,
    provider_ids: &[String],
    now: i64,
    ...
    for provider_id in provider_ids {
        sqlx::query(
            r#"
INSERT IGNORE INTO user_entitlement_providers (
...
async fn apply_plan_debt_repayment_mysql(
    tx: &mut sqlx::Transaction<'_, sqlx::MySql>,
    wallet_id: &str,
    order_id: &str,
    payment_method: &str,
    ...
    let Some(row) = sqlx::query(
        "SELECT status, balance, gift_balance FROM wallets WHERE id = ? LIMIT 1 FOR UPDATE",
    )
    .bind(wallet_id)
    .fetch_optional(&mut **tx)
    .await
    ...
    let after_total = after_recharge + before_gift;
    sqlx::query(
        "UPDATE wallets SET balance = ?, total_recharged = total_recharged + ?, updated_at = ? WHERE id = ?",
    )
    .bind(after_recharge)
    .bind(amount_usd)
    .bind(now)
...
VALUES (?, ?, 'recharge', 'plan_debt_repayment', ?, ?, ?, ?, ?, ?, ?, 'payment_order', ?, NULL, ?, ?)
        "#,
    )
    .bind(uuid::Uuid::new_v4().to_string())
    .bind(wallet_id)
...
async fn mysql_payment_order_by_gateway_order_id_for_update(
    ...
    let sql = payment_order_select_sql("WHERE gateway_order_id = ? LIMIT 1 FOR UPDATE");
    sqlx::query(&sql)
        .bind(gateway_order_id)
        .fetch_optional(&mut **tx)
        .await
...
mod tests {
    ...
    use crate::repository::wallet::{
        AdminPaymentOrderListQuery, AdminRedeemCodeListQuery, AdminWalletListQuery,
    ...
    async fn mysql_wallet_read_repository_reads_wallet_contract_views() {
        let Some(database_url) = std::env::var("AETHER_TEST_MYSQL_URL")
            ...
            eprintln!(
                "skipping mysql wallet read smoke test because AETHER_TEST_MYSQL_URL is unset"
            );
            return;
        };
