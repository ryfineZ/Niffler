...
WHERE user_id = ?
  ...
  AND status = 'active'
                ...
                apply_plan_debt_repayment_sqlite(
                    &mut tx,
                    &order_wallet_id,
                    &order_id,
...
async fn apply_plan_debt_repayment_sqlite(
    tx: &mut sqlx::Transaction<'_, sqlx::Sqlite>,
    wallet_id: &str,
    order_id: &str,
    payment_method: &str,
    ...
    let Some(row) =
        sqlx::query("SELECT status, balance, gift_balance FROM wallets WHERE id = ? LIMIT 1")
            .bind(wallet_id)
            .fetch_optional(&mut **tx)
            .await
            .map_sql_err()?
...
"#,
    ...
    #[tokio::test]
    async fn sqlite_plan_purchase_repays_recorded_debt_and_keeps_new_debt() {
        let pool = sqlx::sqlite::SqlitePoolOptions::new()
            .max_connections(1)
            .connect("sqlite::memory:")
            .await
...
INSERT INTO billing_plans (
  ...
  max_active_per_user, purchase_limit_scope, entitlements_json, created_at, updated_at
        ...
        sqlx::query("UPDATE wallets SET balance = -3 WHERE id = ?")
        ...
        sqlx::query("UPDATE wallets SET balance = -4 WHERE id = ?")
            .bind("wallet-plan-debt-1")
            .execute(repository.pool())
            .await
            .expect("a newly settled request should add one dollar of debt");
        ...
        let WalletMutationOutcome::Applied((credited_order, true)) = outcome else {
        ...
        let balance: f64 =
            sqlx::query_scalar("SELECT balance + gift_balance FROM wallets WHERE id = ?")
                .bind("wallet-plan-debt-1")
                .fetch_one(repository.pool())
                .await
                .expect("wallet balance should query");
        ...
        let balance_after_duplicate: f64 =
            sqlx::query_scalar("SELECT balance + gift_balance FROM wallets WHERE id = ?")
                .bind("wallet-plan-debt-1")
                .fetch_one(repository.pool())
                .await
