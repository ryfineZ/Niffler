...
pub(crate) fn admin_provider_pool_key_hard_error_reason(
    status_code: u16,
    error_body: Option<&str>,
) -> Option<String> {
...
mod tests {
    use super::{
        admin_provider_pool_key_hard_error_reason, parse_google_quota_cooldown_seconds_at,
    ...
    #[test]
    fn hard_error_reason_detects_workspace_deactivated_errors() {
        assert_eq!(
            admin_provider_pool_key_hard_error_reason(
                402,
    ...
    #[test]
    fn hard_error_reason_detects_account_ban_errors() {
        assert_eq!(
            admin_provider_pool_key_hard_error_reason(
                403,
    ...
    #[tokio::test]
    async fn severe_account_errors_do_not_use_transient_pool_cooldown() {
        let Some(redis) = start_managed_redis_or_skip().await else {
            return;
        };
