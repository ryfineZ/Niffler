...
进度：实现和本地审查完成。270 项 Codex、41 项号池调度、167 项架构测试通过（过滤集合有重叠）；完整前端 663 项测试、类型检查、生产构建、Gateway Clippy（`-D warnings`）、格式及 diff 检查通过。覆盖零请求账号自动发现/采集、130 账号分页、目录账号闲置保留与移除、末页优先、路由白名单、模型容量冷却、陈旧索引正常回退、多实例出口与保护隔离。测试未调用真实 OAuth 上游。CI 曾检出号池整页测试仍断言旧文案，已同步“可优先使用”、适用模型和待采集断言；下一步完成 CI 和精确提交镜像发布两台应用服务器，不要求先进入 test 环境。生产前端仍有既存构建分块大小提示，不影响构建通过。
