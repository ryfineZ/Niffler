...
日期：2026-09-20。状态：首版已发布；Compact 协议兼容与同出口复用修复已合入 main；无 state 兜底修复正在验证，待一并发布至两台应用服务器。实际发布版本以各主机 `.niffler-deployed-commit` 和镜像 revision 为准。
...
首版采用全局布尔配置 `codex_turn_state_enabled`，默认关闭；开启后默认使用 `codex_turn_state_fallback=passthrough`，没有可用 state 时跳过注入并按原配置发送请求，不把采集失败变成全局硬失败。需要强制要求 state 时可明确设置 `strict`。采集使用已选账号当前执行计划中的出口，一轮只探测该出口一次，最多 20 秒，失败冷却 180 秒；不自动扩大该账号允许的出口范围。只保留按需主用状态，不启动备用预热或后台服务。独立随机出口、订阅管理和备用提前采集留待实际需要时扩展。
